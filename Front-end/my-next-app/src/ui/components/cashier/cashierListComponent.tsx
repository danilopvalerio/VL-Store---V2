...
const CaixaCard = ({ caixa, onSelect }) => {
  ...
  return (
    <div className={`${styles.box} ${styles.boxHover} ${isAberto ? styles.borderLeftSuccess : styles.borderLeftDanger} ${styles.cursorPointer}`} onClick={onSelect}>
...
const CashierList = ({ caixas, onSelectCaixa }) => (
  <>
    ...
    <div className={`${styles.box} ${styles.mb6}`}>
      <div className={`${styles.grid} ${styles.gap4} ${styles.itemsCenter}`} style={{gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))'}}>
        <div className={styles.formGroup}>
          ...
          <select className={styles.inputForm}><option>Todos</option><option>Aberto</option><option>Fechado</option></select>
        </div>
        <div className={styles.formGroup}>
    ...
    <div className={`${styles.grid} ${styles.gap4}`} style={{gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))'}}>
      {caixas.map((caixa) => <CaixaCard key={caixa.id} caixa={caixa} onSelect={() => onSelectCaixa(caixa)} />)}
    </div>
  </>
);
