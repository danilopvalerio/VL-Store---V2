          ...
          <div className="row mb-3">
            ...
            <div className="col-md-6">
              ...
              <select id="vendedorResponsavel" className="form-control custom-select input-form" value={vendedorResponsavelId} onChange={(e) => setVendedorResponsavelId(e.target.value)} required>
                ...
                {vendedoresDisponiveis.map(vendedor => (<option key={vendedor.id_funcionario} value={vendedor.id_funcionario} className="text-black">{vendedor.nome} {vendedor.cargo ? `(${vendedor.cargo})` : ''}</option>))}
              </select>
            </div>
          </div>
          <div className="row mb-4">
            ...
            <div className="col-md-6">
              ...
              <select id="formaPagamento" className="form-control custom-select input-form" value={formaPagamento} onChange={(e) => setFormaPagamento(e.target.value)} required>
                ...
                <option value="Cartao_Credito" className="text-black">Cartão de Crédito</option>
                <option value="Cartao_Debito" className="text-black">Cartão de Débito</option>
          ...
          <div className="row align-items-end mb-3">
            <div className="col-md-5">
              ...
              <select id="produtoVenda" className="form-control custom-select input-form" value={produtoSelecionadoId} onChange={(e) => setProdutoSelecionadoId(e.target.value)}>
                <option value="" disabled className="text-gray-500">Selecione...</option>
                {produtosDisponiveis.map(p => (<option key={p.id_variacao} value={p.id_variacao} className="text-black">{p.produto.nome} (REF: {p.produto.referencia}) - {p.descricao_variacao}</option>))}
              </select>
            </div>
            ...
            <div className="col-md-2">
              <button type="button" className="btn btn-block primaria" style={{height: '38px', marginTop: '6px'}} onClick={handleAdicionarProdutoVenda}><i className="fas fa-plus"></i>Adicionar</button>
            </div>
          </div>
              ...
              <tbody>
                {carrinhoVenda.length === 0 ? (<tr><td colSpan={5} className="text-center py-3 text-white-75">Nenhum produto adicionado.</td></tr>) : (
                  carrinhoVenda.map(item => {
                    ...
                    return (<tr key={item.id_variacao} className="fine-transparent-border"><td>{item.nome} <small>({item.descricao_variacao})</small></td><td>{item.quantidade}</td><td>R$ {item.precoUnitario.toFixed(2)}</td><td>R$ {subtotalItem.toFixed(2)}</td><td><button type="button" className="btn btn-delete" onClick={() => handleRemoverProdutoDoCarrinho(item.id_variacao)} title="Remover"><i className="fas fa-trash-alt"></i></button></td></tr>);
                  })
                )}
              </tbody>
            </table>
