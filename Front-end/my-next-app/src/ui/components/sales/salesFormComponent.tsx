...
useEffect(() => {
  const fetchCaixasDoVendedor = async () => {
    ...
    // 1. Encontrar o objeto completo do vendedor selecionado para obter o id_loja
  ...
  // --- LOG 1: VER A RESPOSTA BRUTA DA API ---
  console.log("RESPOSTA DA API DE CAIXAS:", response.data.data);

  const caixasDoVendedor = response.data.data.filter(
    (caixa: any) =>
      caixa.id_funcionario_responsavel === vendedorResponsavelId && caixa.status === 'ABERTO'
  );

  // --- LOG 2: VER O RESULTADO DO FILTRO ---
  console.log("CAIXAS FILTRADOS:", caixasDoVendedor);

  setCaixasDisponiveis(caixasDoVendedor);

  if (caixasDoVendedor.length === 1) {
    setCaixaSelecionadoId(caixasDoVendedor[0].id_caixa);
  } else if (caixasDoVendedor.length === 0) {
    showMessage("Este vendedor não possui um caixa aberto nesta loja.", "error");
  }
  ...
  fetchCaixasDoVendedor();
}, [vendedorResponsavelId, jwtToken, vendedoresDisponiveis]);
...
const handleSubmitVenda = async (event: React.FormEvent<HTMLFormElement>) => {
  ...
  try {
    ...
    const movimentacaoPayload = {
      ...
      descricao: `Venda #${vendaCriada.codigo_venda || idVendaCriada.substring(0, 8)}`, // Descrição mais informativa
  ...
  return (
    <div className="quinary text-white p-4 rounded-5 white-light-small d-flex flex-column w-75 mx-auto h-100">
      ...
      <div className="flex-grow-1" style={{ overflowY: "auto", paddingRight: "10px" }}>
        <form id="sales-form-content" onSubmit={handleSubmitVenda} noValidate>
            ...
            <div className="col-md-6 position-relative">
              ...
              {showVendedorDropdown && (
                <ul className="list-group position-absolute w-100 mt-1 z-index-dropdown bg-dark border rounded">
                  {(vendedorSearchTerm && filteredVendedores.length > 0 ? filteredVendedores : vendedoresDisponiveis).map((v) => (
                      <li key={v.id_funcionario}
          ...
          <div className="row mb-4">
            ...
            {/* Caixa */}
            <div className="col-md-6">
              <label htmlFor="caixa" className="form-label text-white-75 small">Caixa</label>
              <select id="caixa" className="form-control custom-select input-form" value={caixaSelecionadoId} onChange={(e) => setCaixaSelecionadoId(e.target.value)}
                disabled={!vendedorResponsavelId || isLoadingCaixas || caixasDisponiveis.length === 0} required>
                <option value="" disabled>
                  {isLoadingCaixas ? "Carregando caixas..." : (vendedorResponsavelId ? "Selecione um caixa aberto..." : "Selecione um vendedor")}
                </option>
                {caixasDisponiveis.map((caixa) => (
                  <option key={caixa.id_caixa} value={caixa.id_caixa}>{caixa.id_caixa}</option>
                ))}
              </select>
              {!isLoadingCaixas && vendedorResponsavelId && caixasDisponiveis.length === 0 && (
                  <div className="text-danger small mt-1">Nenhum caixa aberto para este vendedor.</div>
              )}
            </div>
          </div>
          ...
          {/* ... restante do JSX para adicionar e listar produtos ... */}
           ...
           <div className="row g-2 align-items-baseline mb-3">
            ...
            <div className="col-md-5 position-relative">
              <label htmlFor="produtoVenda" className="form-label text-white-75 small mb-1">Produto</label>
              <input type="text" className="form-control input-form" placeholder="Buscar produto..." value={produtoSearchTerm}
                onChange={(e) => {
              ...
              {showProductDropdown && filteredProducts.length > 0 && (
                <ul className="list-group position-absolute w-100 mt-1 z-index-dropdown bg-dark border rounded">
                  {filteredProducts.map((p) => (
                    <li key={p.id_variacao} className="list-group-item bg-dark text-white cursor-pointer hover-light"
                      onClick={() => {
            ...
            <div className="col-md-2">
              <label htmlFor="quantidadeProduto" className="form-label text-white-75 small mb-1">Qtd.</label>
              <input type="number" className="form-control input-form" id="quantidadeProduto" value={quantidadeProduto}
                onChange={(e) => setQuantidadeProduto(parseInt(e.target.value, 10) || 1)} min="1"/>
            </div>
            {/* Preço Unitário */}
            <div className="col-md-2">
              <label htmlFor="precoUnitario" className="form-label text-white-75 small mb-1">Preço Un.</label>
              <input type="text" className="form-control input-form" id="precoUnitario" value={precoUnitario ? `R$ ${precoUnitario}` : ""} readOnly/>
            </div>
            {/* Botão Adicionar */}
            <div className="col-md-3 d-flex align-items-end">
              <button type="button" className="btn primaria w-100" onClick={handleAdicionarProdutoVenda}>
                <i className="fas fa-plus"></i> Adicionar
              </button>
            </div>
          </div>
          {/* Tabela de Produtos do Carrinho */}
          <div className="mb-4 table-responsive quartenary p-2 rounded-lg" style={{ maxHeight: "200px", overflowY: "auto" }}>
            <table className="table table-sm table-borderless text-white">
              <thead>
                <tr className="fine-transparent-border">
                  <th>Produto</th><th>Qtd</th><th>Preço Un.</th><th>Subtotal</th><th>Ação</th>
                </tr>
              </thead>
              <tbody>
                {carrinhoVenda.length === 0 ? (
                  <tr><td colSpan={5} className="text-center py-3 text-white-75">Nenhum produto adicionado.</td></tr>
                ) : (
                  carrinhoVenda.map((item) => (
                    <tr key={item.id_variacao} className="fine-transparent-border">
                      <td>{item.nome} <small>({item.descricao_variacao})</small></td>
                      <td>{item.quantidade}</td>
                      <td>R$ {item.precoUnitario.toFixed(2)}</td>
                      <td>R$ {(item.quantidade * item.precoUnitario).toFixed(2)}</td>
                      <td>
                        <button type="button" className="btn btn-delete" onClick={() => handleRemoverProdutoDoCarrinho(item.id_variacao)} title="Remover">
